    ...
    <>
      ...
      <nav className="fixed block top-8 right-8 pointer-events-auto md:hidden z-20">
        ...
        <Dialog as="div" onClose={() => setModalOpen(false)} open={modalOpen}>
          ...
          <div
            className="fixed inset-0 z-50 bg-zinc-800/50 backdrop-blur opacity-100"
            aria-hidden="true"
          />
          <Dialog.Panel className="flex flex-row fixed mx-4 inset-x-4 top-8 z-50 origin-top rounded-3xl bg-white p-4 ring-1 ring-zinc-900/5 opacity-100 scale-100 shadow-lg shadow-slate-800/5 backdrop-blur">
            <ul className="flex w-full flex-col text-sm pt-4">
              <li>
                <a
                  className="relative block px-3 py-2 transition hover:text-blue-500"
                  href="/"
